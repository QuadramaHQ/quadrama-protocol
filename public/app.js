...
let lockReason = '';
// Count-only: messages received while keyChangedLock is active are deliberately
// dropped (we cannot safely replay them — the ratchet state may have moved on
// and silently surfacing pre-trust ciphertext could mask a MITM). The counter
// only exists so the UI can tell the user how many were lost.
let droppedWhileLockedCount = 0;
...
// Cryptographically-strong helper around the WebCrypto RNG. We deliberately
// avoid Math.random anywhere user-visible state could leak — even non-secret
// IDs benefit from full 8-char entropy.
...
function secureRandomInt(min, max) {
  ...
  if (b <= a) return a;
  const range = b - a + 1;
  // Rejection-sample a uint32 into [0, range) for an unbiased draw.
...
const clientId = (() => {
  ...
  if (!v) {
    ...
    v = Array.from(bytes, (b) => b.toString(16).padStart(2, '0')).join('').slice(0, 8);
...
function genRoomCode(len = 8) {
  // 32-char alphabet without visually confusable glyphs (0/O/1/l/i).
  const alphabet = 'abcdefghjkmnpqrstuvwxyz23456789';
  let out = '';
  for (let i = 0; i < len; i++) out += alphabet[secureRandomInt(0, alphabet.length - 1)];
  return out;
}
...
function renderStats() {
  ...
  const lockedDrops = droppedWhileLockedCount;
  ...
  const basic = `locked_drop=${lockedDrops} | kc=${pendKc} | chunks=${chunks} | ${drops} | ${rl}`;
...
const DR = {
  ...
  SEEN: new Set(),
  MAX_SKIP: 50,
  // Defence-in-depth global cap on the skipped-keys map. MAX_SKIP bounds the
  // skips *per call*, but a malicious authenticated peer could otherwise
  // accumulate up to MAX_SKIP keys per DH ratchet step and so grow the map
  // without bound. Map preserves insertion order; oldest entries are dropped
  // on overflow. The cap is far above any legitimate scenario (a peer is at
  // most ~MAX_SKIP messages behind in normal use).
...
async function drSkipMessageKeys(until) {
  ...
  // Cap the global skipped-keys map size. Drop oldest first (FIFO via Map
  // insertion order). See MAX_SKIPPED_TOTAL comment on DR for rationale.
  while (DR.MKSKIPPED.size > DR.MAX_SKIPPED_TOTAL) {
...
function loadOrGenSignKp() {
  ...
  if (encrypted && _sessionPass) {
    try {
      ...
      // Validate the encrypted blob round-trips, but do NOT keep the decrypted
      // secret in the long-lived mySignKp object. All signing routes through
      // withPrivateKey(), which decrypts on demand and zeros after use, so the
      // unencrypted Ed25519 secret only lives in JS heap for a few µs per call.
      const verify = decryptSecret(obj, _sessionPass);
      for (let i = 0; i < verify.length; i++) verify[i] = 0;
      mySignKp = { publicKey: b64dec(obj.pk) };
  ...
  // Generate fresh sign keypair + 32-byte password, then encrypt the secret
  // into sessionStorage and immediately wipe the in-RAM copy. Only the
  // public key is retained in mySignKp; signing pulls the secret on demand
  // via withPrivateKey() and zeros it after each use.
  const fresh = nacl.sign.keyPair();
  _sessionPass = nacl.randomBytes(32);

  const encryptedObj = encryptSecret(fresh.secretKey, _sessionPass);
  encryptedObj.pk = b64enc(fresh.publicKey);
  store.setItem('mm3_encrypted_sign', JSON.stringify(encryptedObj));

  for (let i = 0; i < fresh.secretKey.length; i++) fresh.secretKey[i] = 0;
  mySignKp = { publicKey: fresh.publicKey };
...
async function onChat(msg, fromKcFlush = false) {
  ...
  if (keyChangedLock && !fromKcFlush) {
    // SECURITY: silently dropping ciphertext is *safer* than buffering and
    // replaying it after re-verification. The ratchet/key state during a
    // lock is by definition untrusted; replaying old ciphertext could mask
    // a key-substitution attempt. The user is told a message was discarded.
    droppedWhileLockedCount += 1;
    renderStats();
    return log('[locked] Nachricht verworfen (Lock aktiv – bitte Peer neu verifizieren und Peer um Neuversand bitten).');
  }
...
// UI wiring
// =======================================================
// Default the WS URL to the origin that served the page (I-04). The HTML
// keeps "wss://quadrama.ch/ws" as a no-JS fallback, but as soon as the app
// boots we prefer same-origin so a self-host user does not accidentally
// connect to the reference public relay. The field remains editable.
function initWsUrlDefault() {
  if (!els.wsUrl) return;
  try {
    const loc = (typeof location !== 'undefined') ? location : null;
    if (!loc || !loc.host) return;
    const proto = (loc.protocol === 'https:') ? 'wss:' : 'ws:';
    els.wsUrl.value = `${proto}//${loc.host}/ws`;
  } catch {
    // Keep the HTML fallback if anything throws.
  }
}
